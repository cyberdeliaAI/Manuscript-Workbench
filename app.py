...
<main>
  ...
  <div class="sidebar">
    <div class="side-tabs" role="tablist" aria-label="Workbench tools">
      ...
      <button data-side-tab="vellum" type="button">Markdown</button>
        ...
        <div class="lm-panel">
          ...
          <details class="ai-section" open>
            ...
            <div class="ai-section-body">
              ...
              <div class="lm-grid"><button id="rewriteSelBtn" class="primary">Rewrite Selection</button><button id="rewriteParaBtn">Current Paragraph</button><button id="rewriteWholeBtn">Rewrite Whole</button><button id="rewriteChunkBtn">Rewrite Chunked</button><button id="cleanSelBtn">AI Clean Selection</button><button id="cleanParaBtn">AI Clean Paragraph</button><button id="cleanWholeBtn">AI Clean Whole</button><button id="cleanChunkBtn">AI Clean Chunked</button></div>
            </div>
          </details>
          <details class="ai-section" open>
...
<div class="modal" id="previewModal">
  <div class="modal-card" id="modalCard">
    ...
    <div class="modal-foot"><span class="lm-status" id="previewStatus">Ready.</span><div class="modal-actions"><button id="stopBtn" class="danger" disabled>Stop generation</button><button id="applyPreviewBtn" class="primary" disabled>Apply change</button></div></div>
  </div>
</div>
...
function renderOutline(){ const list=$('#outlineList'); if(!list) return; const rows=headingOutline(editor.value); list.innerHTML=''; if(!rows.length){ list.innerHTML='<div class="empty-hint">No # headings yet. Mark chapters with # to build an outline.</div>'; return; } rows.slice(0,120).forEach(row=>{ const div=document.createElement('div'); div.className='outline-row'+(row.level>1?' sub':''); const tag=row.level===1?'CH':`H${row.level}`; div.innerHTML=`<span class="outline-tag">${tag}</span><div class="outline-main"><div class="outline-title">${escapeHtml(row.title||'(untitled)')}</div><div class="outline-meta">line ${row.line} · ${row.words.toLocaleString()} words</div></div><div class="outline-actions"><button data-act="jump">Go</button>${row.level===1?'<button data-act="rewrite">AI</button><button data-act="clean">Clean</button>':''}</div>`; div.querySelector('[data-act="jump"]').addEventListener('click',()=>jumpToOffset(row.start)); const rewrite=div.querySelector('[data-act="rewrite"]'); if(rewrite) rewrite.addEventListener('click',()=>{ selectChapterRange(row); startLM('rewrite','selection',false); }); const clean=div.querySelector('[data-act="clean"]'); if(clean) clean.addEventListener('click',()=>{ selectChapterRange(row); startLM('clean','selection',false); }); list.appendChild(div); }); if(rows.length>120){ const more=document.createElement('div'); more.className='empty-hint'; more.textContent=`Showing first 120 of ${rows.length} headings.`; list.appendChild(more); } }
...
function openFindModal(){ const s=editor.selectionStart,e=editor.selectionEnd, selected=editor.value.slice(s,e); findScopeRange=s!==e?{start:s,end:e}:null; $('#findScope').value=findScopeRange?'selection':'all'; if(selected && selected.length<=120 && !selected.includes('\n')) $('#findText').value=selected; $('#findModal').classList.add('open'); setTimeout(()=>{$('#findText').focus(); $('#findText').select(); refreshFindMatches(editor.selectionEnd);},0); }
...
function applyInline(kind){ const s=editor.selectionStart,e=editor.selectionEnd; if(s===e){ toast('Select text first.'); return; } const sel=editor.value.slice(s,e); let out=sel; if(kind==='italic') out=`*${sel}*`; else if(kind==='bold') out=`**${sel}**`; else if(kind==='mono') out='`'+sel.replace(/`/g,'')+'`'; else out=`[[${kind}:${sel}]]`; replaceRange(s,e,out); }
...
$('#rewriteSelBtn').addEventListener('click',()=>startLM('rewrite','selection',false)); $('#rewriteParaBtn').addEventListener('click',()=>startLM('rewrite','paragraph',false)); $('#rewriteWholeBtn').addEventListener('click',()=>startLM('rewrite','whole',false)); $('#rewriteChunkBtn').addEventListener('click',()=>startLM('rewrite','whole',true)); $('#cleanSelBtn').addEventListener('click',()=>startLM('clean','selection',false)); $('#cleanParaBtn').addEventListener('click',()=>startLM('clean','paragraph',false)); $('#cleanWholeBtn').addEventListener('click',()=>startLM('clean','whole',false)); $('#cleanChunkBtn').addEventListener('click',()=>startLM('clean','whole',true));
...
async function startLMChunked(task, range, promptText){ const maxChars=Math.max(1200, parseInt($('#chunkSize').value,10)||6000); const chunks=makeChunks(range.text,maxChars); openPreview(`${task==='clean'?'AI Clean':'Rewrite'} chunked`, range.text, '', range.start, range.end, true); previewState.expectedMarkers=markerSignature(range.text); setPreviewProgress(0,chunks.length); for(let i=0;i<chunks.length;i++){ if(!previewState.running) break; const pct=progressPercent(i,chunks.length); $('#streamState').textContent=`${pct}% · chunk ${i+1}/${chunks.length}`; $('#previewStatus').textContent=`Generating chunk ${i+1} of ${chunks.length} (${pct}% done)...`; if(i>0) previewState.after += '\n\n'; const prep=prepareTextForLm(chunks[i]); await streamOne(promptText, buildLmUserContent(prep.text, prep, task), txt=>{ previewState.after += txt; updatePreview(); }, `${task} chunk ${i+1}/${chunks.length}`); setPreviewProgress(i+1,chunks.length); }
...
function openPreview(title,before,after,start,end,running,applyMode='replace'){ previewState={title,before,after,start,end,finished:!running,running,partial:false,expectedMarkers:null,applyMode,progress:null}; clearIntegrityGuardrails(); $('#previewTitle').textContent=title; $('#previewSub').textContent=previewSubText(); $('#beforeBox').textContent=before; $('#afterBox').textContent=after; $('#previewModal').classList.add('open'); setPreviewView(localStorage.getItem('vp.py.previewView')||'split'); $('#applyPreviewBtn').disabled=running || !after || applyMode==='none'; setStopButtons(running); setLmBusy(running); $('#showPreviewBtn').disabled=false; $('#streamState').textContent=running?'generating...':'ready'; $('#previewStatus').textContent=running?'Generating...':(applyMode==='none'?'Review ready.':'Preview ready.'); }
function updatePreview(){ $('#afterBox').textContent=previewState.after; $('#previewSub').textContent=previewSubText(); if($('#autoScroll').checked){ const b=$('#afterBox'); b.scrollTop=b.scrollHeight; } }
