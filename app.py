...
function makeHeadingSections(text){ const lines=String(text||'').split('\n'); const sections=[]; let buf=[]; const push=()=>{ if(buf.length){ const section=buf.join('\n'); if(section.trim()) sections.push(section); buf=[]; } }; for(const line of lines){ const isHeading=/^\s{0,3}#{1,6}\s+\S/.test(line); if(isHeading && buf.length) push(); buf.push(line); } push(); return sections.length ? sections : (String(text||'').trim() ? [String(text||'')] : []); }
async function startLMSectioned(task, scope){ if(previewState.running){ toast('Generation is already running. Stop it first.'); return; } try{ const range=getScopeRange(scope); const promptText=promptForTask(task); if(!range.text.trim()) throw new Error('No text to send.'); const sections=makeHeadingSections(range.text); openPreview(`${task==='clean'?'AI Clean':'Rewrite'} by heading ${scope}`, range.text, '', range.start, range.end, true); previewState.expectedMarkers=markerSignature(range.text); setPreviewProgress(0,sections.length); for(let i=0;i<sections.length;i++){ if(!previewState.running) break; const pct=progressPercent(i,sections.length); $('#streamState').textContent=`${pct}% · section ${i+1}/${sections.length}`; $('#previewStatus').textContent=`Generating heading section ${i+1} of ${sections.length} (${pct}% done)...`; if(i>0) previewState.after += '\n\n'; const prep=prepareTextForLm(sections[i]); await streamOne(promptText, buildLmUserContent(prep.text, prep, task), txt=>{ previewState.after += txt; updatePreview(); }, `${task} heading section ${i+1}/${sections.length}`); setPreviewProgress(i+1,sections.length); } finishSectionedPreview('Heading-based generation'); }catch(err){ if(err?.name==='AbortError' || previewState.partial){ markLmStopped(); return; } toast(err.message); $('#lmStatus').textContent=err.message; finishPreviewError(err.message); } }
async function startGuidedSectioned(kind, scope){ if(previewState.running){ toast('Generation is already running. Stop it first.'); return; } try{ const range=getScopeRange(scope); if(!range.text.trim()) throw new Error('No text to send.'); const sections=makeHeadingSections(range.text); const titles={translate:'Translate by Heading','guided-rewrite':'Rewrite with Guidance by Heading'}; openPreview(titles[kind]||'AI by Heading', range.text, '', range.start, range.end, true, 'replace'); previewState.expectedMarkers=markerSignature(range.text); setPreviewProgress(0,sections.length); for(let i=0;i<sections.length;i++){ if(!previewState.running) break; const pct=progressPercent(i,sections.length); $('#streamState').textContent=`${pct}% · section ${i+1}/${sections.length}`; $('#previewStatus').textContent=`Generating heading section ${i+1} of ${sections.length} (${pct}% done)...`; if(i>0) previewState.after += '\n\n'; await streamOne(guidedSystemPrompt(kind), guidedUserContent(kind, sections[i]), txt=>{ previewState.after += txt; updatePreview(); }, `${kind} heading section ${i+1}/${sections.length}`); setPreviewProgress(i+1,sections.length); } finishSectionedPreview('Heading-based generation'); }catch(err){ if(err?.name==='AbortError' || previewState.partial){ markLmStopped(); return; } toast(err.message); $('#lmStatus').textContent=err.message; finishPreviewError(err.message); } }
function finishSectionedPreview(label){ previewState.after=sanitizeLmFinalText(previewState.after); updatePreview(); previewState.finished=!previewState.partial && previewState.running; previewState.running=false; setStopButtons(false); setLmBusy(false); const hasOutput=!!previewState.after.trim(); const finalPct=previewState.progress?.percent ?? 0; $('#streamState').textContent=previewState.partial?`stopped · ${finalPct}%`:'done · 100%'; $('#applyPreviewBtn').disabled=!!previewState.partial || !hasOutput; $('#previewStatus').textContent=previewState.partial?`Stopped at ${finalPct}%. Partial output is not applicable.`:(hasOutput?`${label} complete.`:`${label} produced no applicable text.`); $('#lmStatus').textContent=$('#previewStatus').textContent; if(!previewState.partial && hasOutput){ renderIntegrityGuardrails(previewState.before,previewState.after); reportPreviewMarkerDrift(); } }
