...
textarea.prompt{height:190px;resize:vertical;font-family:var(--mono);line-height:1.35}.lm-status{font-family:var(--mono);font-size:11px;color:var(--muted);min-height:16px}.prompt-actions{display:grid;grid-template-columns:1fr 1fr;gap:6px}.lm-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px}.lm-subgroup{border:1px solid var(--line-soft);border-radius:7px;background:#191814;padding:8px;display:flex;flex-direction:column;gap:7px}.lm-subgroup-title{font-family:var(--mono);font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:var(--amber)}.footer-note{font-size:10px;color:var(--faint);line-height:1.35;margin-top:8px}
</style>
</head>
<body>
<header>
          ...
          <div class="ai-block">
            ...
            <details class="ai-section" open>
              ...
              <div class="ai-section-body">
                ...
                <div class="lm-subgroup"><div class="lm-subgroup-title">Rewrite</div><div class="lm-grid"><button id="rewriteSelBtn" class="primary">Rewrite Selection</button><button id="rewriteParaBtn">Current Paragraph</button><button id="rewriteWholeBtn">Rewrite Whole</button><button id="rewriteChunkBtn" title="Uses the current selection if there is one; otherwise rewrites the whole text in chunks.">Rewrite Chunked</button><button id="rewriteHeadingBtn" title="Uses the current selection if there is one; otherwise rewrites the whole text section by section at Markdown headings (# through ######).">Rewrite by Heading</button></div></div>
                <div class="lm-subgroup"><div class="lm-subgroup-title">AI Clean</div><div class="lm-grid"><button id="cleanSelBtn">AI Clean Selection</button><button id="cleanParaBtn">AI Clean Paragraph</button><button id="cleanWholeBtn">AI Clean Whole</button><button id="cleanChunkBtn" title="Uses the current selection if there is one; otherwise cleans the whole text in chunks.">AI Clean Chunked</button></div></div>
              </div>
            </details>
            <details class="ai-section" open>
              ...
              <div class="ai-section-body">
                ...
                <div class="lm-grid"><button id="askImproveBtn">Improve Questions</button><button id="feedbackOnlyBtn">Feedback Only</button><button id="rewriteGuidedBtn" class="primary">Rewrite with Guidance</button><button id="translateSelBtn">Translate Selection</button><button id="translateHeadingBtn" title="Uses the current selection if there is one; otherwise translates the whole text section by section at Markdown headings (# through ######).">Translate by Heading</button></div>
...
$('#translateHeadingBtn').addEventListener('click',()=>startGuidedSectioned('translate',chunkScope()));
function setLmBusy(on){ ['rewriteSelBtn','rewriteParaBtn','rewriteWholeBtn','rewriteChunkBtn','rewriteHeadingBtn','cleanSelBtn','cleanParaBtn','cleanWholeBtn','cleanChunkBtn','askImproveBtn','feedbackOnlyBtn','rewriteGuidedBtn','translateSelBtn','translateHeadingBtn'].forEach(id=>{$('#'+id).disabled=on;}); }
function noteStreamFinish(info){ (info?.finishReasons||[]).forEach(reason=>{ if(!previewState.finishReasons) previewState.finishReasons=[]; previewState.finishReasons.push(reason); }); }
function finishWarningText(){ const reasons=previewState.finishReasons||[]; if(reasons.includes('length')) return 'Warning: the provider stopped because the output hit a length limit; the last section may be incomplete.'; if(reasons.includes('content_filter')) return 'Warning: the provider stopped because of a content filter.'; return ''; }
...
function makeChunks(text, maxChars){ const headingSections=makeHeadingSectionsNoFallback(text); const chunks=[]; let largeParagraphs=0; headingSections.forEach(section=>{ if(maxChars>0 && section.length>maxChars*1.25){ const parts=makeParagraphChunks(section,maxChars); largeParagraphs += parts.largeParagraphCount||0; chunks.push(...parts); } else { chunks.push(section); } }); chunks.largeParagraphCount=largeParagraphs; return chunks.filter(chunk=>String(chunk||'').trim()); }
function splitParagraphBlocks(text){ const lines=String(text||'').split('\n'); const blocks=[]; let buf=[]; const push=()=>{ if(buf.length){ blocks.push(buf.join('\n').trimEnd()); buf=[]; } }; for(const line of lines){ if(line.trim()===''){ push(); } else { buf.push(line); } } push(); return blocks.filter(block=>block.trim()); }
function makeParagraphChunks(text,maxChars){ const blocks=splitParagraphBlocks(text); if(!blocks.length) return String(text||'').trim() ? [String(text||'')] : []; const chunks=[]; let buf=[], len=0, huge=0; const push=()=>{ if(buf.length){ chunks.push(buf.join('\n\n')); buf=[]; len=0; } }; blocks.forEach(block=>{ const addLen=block.length+(buf.length?2:0); if(maxChars>0 && len>0 && len+addLen>maxChars) push(); if(maxChars>0 && block.length>maxChars*1.25) huge++; buf.push(block); len += addLen; }); push(); chunks.largeParagraphCount=huge; return chunks; }
function makeHeadingSectionsNoFallback(text){ const lines=String(text||'').split('\n'); const rawSections=[]; let buf=[]; const push=()=>{ if(buf.length){ const section=buf.join('\n'); if(section.trim()) rawSections.push(section); buf=[]; } }; for(const line of lines){ const isHeading=/^\s{0,3}#{1,6}\s+\S/.test(line); if(isHeading && buf.length) push(); buf.push(line); } push(); return rawSections.length ? rawSections : (String(text||'').trim() ? [String(text||'')] : []); }
function makeHeadingSections(text,maxChars=0){ const base=makeHeadingSectionsNoFallback(text); const sections=[]; let oversized=0, largeParagraphs=0; base.forEach(section=>{ if(maxChars>0 && section.length>maxChars*1.25){ const parts=makeParagraphChunks(section,maxChars); largeParagraphs += parts.largeParagraphCount||0; if(parts.length>1){ oversized++; sections.push(...parts); return; } } sections.push(section); }); sections.oversizedCount=oversized; sections.largeParagraphCount=largeParagraphs; sections.originalCount=base.length; return sections; }
async function startLMSectioned(task, scope){ if(previewState.running){ toast('Generation is already running. Stop it first.'); return; } try{ const range=getScopeRange(scope); const promptText=promptForTask(task); if(!range.text.trim()) throw new Error('No text to send.'); const maxChars=Math.max(1200, parseInt($('#chunkSize').value,10)||6000); const sections=makeHeadingSections(range.text,maxChars); openPreview(`${task==='clean'?'AI Clean':'Rewrite'} by heading ${scope}`, range.text, '', range.start, range.end, true); previewState.expectedMarkers=markerSignature(range.text); previewState.sectionSplitNote=headingSplitNote(sections); setPreviewProgress(0,sections.length); for(let i=0;i<sections.length;i++){ if(!previewState.running) break; const pct=progressPercent(i,sections.length); $('#streamState').textContent=`${pct}% · section ${i+1}/${sections.length}`; $('#previewStatus').textContent=`Generating heading section ${i+1} of ${sections.length} (${pct}% done)...`; if(i>0) previewState.after += '\n\n'; const prep=prepareTextForLm(sections[i]); const info=await streamOne(promptText, buildLmUserContent(prep.text, prep, task), txt=>{ previewState.after += txt; updatePreview(); }, `${task} heading section ${i+1}/${sections.length}`); noteStreamFinish(info); setPreviewProgress(i+1,sections.length); } finishSectionedPreview('Heading-based generation'); }catch(err){ if(err?.name==='AbortError' || previewState.partial){ markLmStopped(); return; } toast(err.message); $('#lmStatus').textContent=err.message; finishPreviewError(err.message); } }
async function startGuidedSectioned(kind, scope){ if(previewState.running){ toast('Generation is already running. Stop it first.'); return; } try{ const range=getScopeRange(scope); if(!range.text.trim()) throw new Error('No text to send.'); const maxChars=Math.max(1200, parseInt($('#chunkSize').value,10)||6000); const sections=makeHeadingSections(range.text,maxChars); const titles={translate:'Translate by Heading','guided-rewrite':'Rewrite with Guidance by Heading'}; openPreview(titles[kind]||'AI by Heading', range.text, '', range.start, range.end, true, 'replace'); previewState.expectedMarkers=markerSignature(range.text); previewState.sectionSplitNote=headingSplitNote(sections); setPreviewProgress(0,sections.length); for(let i=0;i<sections.length;i++){ if(!previewState.running) break; const pct=progressPercent(i,sections.length); $('#streamState').textContent=`${pct}% · section ${i+1}/${sections.length}`; $('#previewStatus').textContent=`Generating heading section ${i+1} of ${sections.length} (${pct}% done)...`; if(i>0) previewState.after += '\n\n'; const info=await streamOne(guidedSystemPrompt(kind), guidedUserContent(kind, sections[i]), txt=>{ previewState.after += txt; updatePreview(); }, `${kind} heading section ${i+1}/${sections.length}`); noteStreamFinish(info); setPreviewProgress(i+1,sections.length); } finishSectionedPreview('Heading-based generation'); }catch(err){ if(err?.name==='AbortError' || previewState.partial){ markLmStopped(); return; } toast(err.message); $('#lmStatus').textContent=err.message; finishPreviewError(err.message); } }
function headingSplitNote(sections){ const notes=[]; if(sections.oversizedCount) notes.push(`${sections.oversizedCount} large heading section(s) were split further on whole paragraphs using Chunk size.`); if(sections.largeParagraphCount) notes.push(`${sections.largeParagraphCount} paragraph(s) were larger than Chunk size and were sent whole to avoid cutting a paragraph in half.`); return notes.join(' '); }
function finishSectionedPreview(label){ previewState.after=sanitizeLmFinalText(previewState.after); updatePreview(); previewState.finished=!previewState.partial && previewState.running; previewState.running=false; setStopButtons(false); setLmBusy(false); const hasOutput=!!previewState.after.trim(); const finalPct=previewState.progress?.percent ?? 0; $('#streamState').textContent=previewState.partial?`stopped · ${finalPct}%`:'done · 100%'; $('#applyPreviewBtn').disabled=!!previewState.partial || !hasOutput; const notes=[previewState.sectionSplitNote, finishWarningText()].filter(Boolean).join(' '); $('#previewStatus').textContent=previewState.partial?`Stopped at ${finalPct}%. Partial output is not applicable.`:(hasOutput?`${label} complete.${notes?' '+notes:''}`:`${label} produced no applicable text.${notes?' '+notes:''}`); $('#lmStatus').textContent=$('#previewStatus').textContent; if(!previewState.partial && hasOutput){ renderIntegrityGuardrails(previewState.before,previewState.after); reportPreviewMarkerDrift(); } }
async function startLMChunked(task, scope, range, promptText){ const maxChars=Math.max(1200, parseInt($('#chunkSize').value,10)||6000); const chunks=makeChunks(range.text,maxChars); openPreview(`${task==='clean'?'AI Clean':'Rewrite'} chunked ${scope}`, range.text, '', range.start, range.end, true); previewState.expectedMarkers=markerSignature(range.text); previewState.sectionSplitNote=chunks.largeParagraphCount?`${chunks.largeParagraphCount} paragraph(s) were larger than Chunk size and were sent whole to avoid cutting a paragraph in half.`:''; setPreviewProgress(0,chunks.length); for(let i=0;i<chunks.length;i++){ if(!previewState.running) break; const pct=progressPercent(i,chunks.length); $('#streamState').textContent=`${pct}% · chunk ${i+1}/${chunks.length}`; $('#previewStatus').textContent=`Generating chunk ${i+1} of ${chunks.length} (${pct}% done)...`; if(i>0) previewState.after += '\n\n'; const prep=prepareTextForLm(chunks[i]); const info=await streamOne(promptText, buildLmUserContent(prep.text, prep, task), txt=>{ previewState.after += txt; updatePreview(); }, `${task} chunk ${i+1}/${chunks.length}`); noteStreamFinish(info); setPreviewProgress(i+1,chunks.length); }
 previewState.after=sanitizeLmFinalText(previewState.after); updatePreview(); previewState.finished=!previewState.partial && previewState.running; previewState.running=false; setStopButtons(false); setLmBusy(false); const hasOutput=!!previewState.after.trim(); const finalPct=previewState.progress?.percent ?? 0; const notes=[previewState.sectionSplitNote, finishWarningText()].filter(Boolean).join(' '); $('#streamState').textContent=previewState.partial?`stopped · ${finalPct}%`:'done · 100%'; $('#applyPreviewBtn').disabled=!!previewState.partial || !hasOutput; $('#previewStatus').textContent=previewState.partial?`Stopped at ${finalPct}%. Partial output is not applicable.`:(hasOutput?`Chunked generation complete.${notes?' '+notes:''}`:`Chunked generation produced no applicable text.${notes?' '+notes:''}`); $('#lmStatus').textContent=$('#previewStatus').textContent; if(!previewState.partial && hasOutput){ renderIntegrityGuardrails(previewState.before,previewState.after); reportPreviewMarkerDrift(); } }
function extractStreamMeta(text){ const reasons=[]; const clean=String(text||'').replace(/<<MW_FINISH_REASON:([^>]+)>>/g,(_,reason)=>{ reasons.push(reason); return ''; }); return {text:clean,reasons}; }
async function streamOne(promptText, inputText, onText, label){ const provider=currentAiProvider(); lmAbort=new AbortController(); setStopButtons(true); $('#showPreviewBtn').disabled=false; const info={finishReasons:[]}; const res=await fetch('/api/lm_stream',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({base_url:$('#lmBase').value,provider:$('#lmProvider').value,api_key:$('#lmApiKey').value,model:$('#lmModel').value,prompt:promptText,input:inputText,options:advancedOptions()}),signal:lmAbort.signal}); if(!res.ok){ const msg=await res.text(); throw new Error(msg||`${provider.label} request failed`); } const reader=res.body.getReader(); const decoder=new TextDecoder(); while(true){ const {value,done}=await reader.read(); if(done) break; const chunk=decoder.decode(value,{stream:true}); if(chunk){ const meta=extractStreamMeta(chunk); info.finishReasons.push(...meta.reasons); if(meta.text) onText(meta.text); } } return info; }
            ...
            for raw in resp.iter_lines(decode_unicode=False):
                ...
                try:
                    ...
                    if content:
                        ...
                        self.wfile.flush()
                    if finish_reason:
                        self.wfile.write(f"<<MW_FINISH_REASON:{finish_reason}>>".encode("utf-8"))
                        self.wfile.flush()
